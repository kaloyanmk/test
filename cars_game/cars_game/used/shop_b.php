...
?>
 <form action="shop_b.php" method="post">
...
$shop_type=("SELECT * FROM `shop` WHERE `type`='3'");




$result = $conn->query($shop_type);
echo "<form  action=\"shop_b.php\" method=\"post\">";
...
$name_n=$_SESSION['user_name'];
      for($i=0; $i<$count;$i++)
      {
          if(isset($_POST[$i]))
          {
								mysqli_query($conn,"UPDATE `my_car`SET `brakes`=`brakes`+$var[$i] WHERE `owner`='$name_n'") ;
